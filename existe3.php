...
$proceso2 = $con->query("SELECT * FROM visitas WHERE correo = '$usuario' AND pass = '$password' ");

if($resultado = mysqli_fetch_array($proceso2)){

  $_SESSION['id'] = $resultado['id_visi'];
  $_SESSION['foto'] = $resultado['foto'];
  $_SESSION['nombre'] = $resultado['nombre'];
  $_SESSION['apellidoPat'] = $resultado['app'];
  $_SESSION['apellidoMat'] = $resultado['apm'];
  ...
  //header("Location: alumno.php");
    ...
    echo "<script>alert('Bienvenido al portal visitante, ".$_SESSION['nombre'].", ".$_SESSION['apellidoPat'].", ".$_SESSION['apellidoMat']."');
    location.href='visitas.php';
    </script>";
}else{
  echo '<script>
